...
from hr_crux_automations.methods.methods import get_model_class
from hr_crux_views.cbv_methods import render_template

"""Objectives and key result section"""


class Period(hr_cruxModel):
...
class KeyResult(hr_cruxModel):
    ...
    history = hr_cruxAuditLog(bases=[hr_cruxAuditInfo])
...
class EmployeeObjective(hr_cruxModel):
    ...
    history = hr_cruxAuditLog(bases=[hr_cruxAuditInfo], related_name="history_set")
    archive = models.BooleanField(default=False)
    objects = hr_cruxCompanyManager("employee_id__employee_work_info__company_id")
...
class Comment(models.Model):
    ...
    history = hr_cruxAuditLog(excluded_fields=["comment"], bases=[hr_cruxAuditInfo])
    objects = hr_cruxCompanyManager(
...
class QuestionTemplate(hr_cruxModel):
...
class Question(hr_cruxModel):
    ...
    objects = hr_cruxCompanyManager("template_id__company_id")

    def __str__(self):
        return self.question


class QuestionOptions(hr_cruxModel):
    ...
    objects = hr_cruxCompanyManager("question_id__template_id__company_id")


class Feedback(hr_cruxModel):
...
class EmployeeBonusPoint(hr_cruxModel):
    employee_id = models.ForeignKey(
        Employee,
        on_delete=models.DO_NOTHING,
