...
try:
    ...
    hr_crux_mail_templates = hr_cruxMailTemplate.objects.all()
...
try:
    ...
    for penalty in penalties:
        ...
        if not PenaltyAccounts.objects.filter(**filter_conditions).exists():
            hr_crux = PenaltyAccounts(
                id=penalty.id,
                employee_id=penalty.employee_id,
                penalty_amount=penalty.penalty_amount,
            )
            if apps.is_installed("attendance"):
                hr_crux.late_early_id = penalty.late_early_id
            if apps.is_installed("leave"):
                hr_crux.leave_request_id = penalty.leave_request_id
                hr_crux.leave_type_id = penalty.leave_type_id
                hr_crux.minus_leaves = penalty.minus_leaves
                hr_crux.deduct_from_carry_forward = penalty.deduct_from_carry_forward
            hr_crux.save()
