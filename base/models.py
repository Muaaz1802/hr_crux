...
from base.hr_crux_company_manager import hr_cruxCompanyManager
from hr_crux import hr_crux_middlewares
from hr_crux.hr_crux_middlewares import _thread_locals
from hr_crux.methods import get_hr_crux_model_class
from hr_crux.models import hr_cruxModel
from hr_crux_audit.models import hr_cruxAuditInfo, hr_cruxAuditLog
...
class JobPosition(hr_cruxModel):
    """
    JobPosition model
    """
    ...
    objects = hr_cruxCompanyManager("department_id__company_id")
...
class JobRole(hr_cruxModel):
    ...
    objects = hr_cruxCompanyManager("job_position_id__department_id__company_id")
...
class RotatingWorkType(hr_cruxModel):
    """
    ...
    objects = hr_cruxCompanyManager("employee_id__employee_work_info__company_id")
...
class RotatingWorkTypeAssign(hr_cruxModel):
    """
...
class EmployeeType(hr_cruxModel):
    """
    EmployeeType model
    """
...
class EmployeeShift(hr_cruxModel):
    """
    EmployeeShift model
    """
    ...
    objects = hr_cruxCompanyManager("employee_shift__company_id")
...
class EmployeeShiftSchedule(hr_cruxModel):
    """
    ...
    auto_punch_out_time = models.TimeField(
        ...
        help_text=_(
            "Time at which the hr_crux will automatically check out the employee attendance if they forget."
        ),
    )
    company_id = models.ManyToManyField(Company, blank=True, verbose_name=_("Company"))

    objects = hr_cruxCompanyManager("shift_id__employee_shift__company_id")
...
class RotatingShift(hr_cruxModel):
    """
    RotatingShift model
    """
...
class RotatingShiftAssign(hr_cruxModel):
    """
...
class WorkTypeRequest(hr_cruxModel):
    """
    ...
    def clean(self):
        request = getattr(hr_crux_middlewares._thread_locals, "request", None)
...
class WorkTypeRequestComment(hr_cruxModel):
    """
...
class ShiftRequest(hr_cruxModel):
    """
    ShiftRequest model
    """
...
class ShiftRequestComment(hr_cruxModel):
    """
...
class Tags(hr_cruxModel):
    title = models.CharField(max_length=30)
    color = models.CharField(max_length=30)
    company_id = models.ForeignKey(
        Company, null=True, editable=False, on_delete=models.PROTECT
    )
    objects = hr_cruxCompanyManager(related_company_field="company_id")

    def __str__(self):
        return self.title


class hr_cruxMailTemplate(hr_cruxModel):
...
class DynamicEmailConfiguration(hr_cruxModel):
    """
...
class MultipleApprovalCondition(hr_cruxModel):
...
class Announcement(hr_cruxModel):
    """
...
class AnnouncementComment(hr_cruxModel):
    """
...
class DashboardEmployeeCharts(hr_cruxModel):
    from employee.models import Employee
...
class TrackLateComeEarlyOut(hr_cruxModel):
...
class CompanyLeaves(hr_cruxModel):
...
class PenaltyAccounts(hr_cruxModel):
    """
...
def create_deduction_cutleave_from_penalty(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        if apps.is_installed("payroll") and penalty_amount:
            Deduction = get_hr_crux_model_class(app_label="payroll", model="deduction")
            penalty = Deduction()
