...
from hr_crux import hr_crux_apps
from hr_crux.decorators import (
    delete_permission,
...
from hr_crux.group_by import group_by_queryset
from hr_crux.hr_crux_settings import (
    APPS,
    DB_INIT_PASSWORD,
...
from hr_crux.methods import get_hr_crux_model_class, remove_dynamic_url
from hr_crux_audit.forms import HistoryTrackingFieldsForm
from hr_crux_audit.models import AccountBlockUnblock, AuditTag, HistoryTrackingFields
...
def initialize_database(request):
    ...
    if initialize_database_condition():
        if request.method == "POST":
            ...
            from hr_crux.hr_crux_settings import DB_INIT_PASSWORD as db_password
        ...
        return render(request, "initialize_database/hr_crux_user.html")
    else:
        return redirect("/")
...
def initialize_database_user(request):
    ...
    if request.method == "POST":
        ...
        if password != confirm_password:
            return render(request, "initialize_database/hr_crux_user_signup.html")
        ...
        return render(
            request,
            "initialize_database/hr_crux_company.html",
...
def initialize_database_company(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                "initialize_database/hr_crux_department.html",
                {"form": DepartmentForm(initial={"company_id": company})},
            )
    return render(request, "initialize_database/hr_crux_company.html", {"form": form})
...
def initialize_database_department(request):
    ...
    return render(
        request,
        "initialize_database/hr_crux_department_form.html",
...
def initialize_database_job_position(request):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "initialize_database/hr_crux_job_position_form.html",
            {
                "form": form,
    ...
    return render(
        request,
        "initialize_database/hr_crux_job_position.html",
...
class hr_cruxPasswordResetView(PasswordResetView):
    """
    hr_crux View for Reset Password
    """
...
class EmployeePasswordResetView(PasswordResetView):
    """
    hr_crux View for Employee Reset Password
    """
...
def object_delete(request, obj_id, **kwargs):
    ...
    if apps.is_installed("pms") and redirect_path == "/pms/filter-key-result/":
        KeyResult = get_hr_crux_model_class(app_label="pms", model="keyresult")
...
def mail_server_test_email(request):
    ...
    white_labelling = getattr(hr_crux_apps, "WHITE_LABELLING", False)
    image_path = path.join(settings.STATIC_ROOT, "images/ui/hr_crux-logo.png")
    company_name = "hr_crux"
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            subject = _("Test mail from hr_crux")
...
@login_required
@hx_request_required
@permission_required("base.change_hr_cruxmailtemplate")
def view_mail_template(request, obj_id):
    """
    This method is used to display the template/form to edit
    """
    template = hr_cruxMailTemplate.objects.get(id=obj_id)
...
@login_required
@require_http_methods(["POST"])
@permission_required("base.add_hr_cruxmailtemplate")
...
@login_required
@permission_required("base.delete_hr_cruxmailtemplate")
def delete_mail_templates(request):
    ids = request.GET.getlist("ids")
    result = hr_cruxMailTemplate.objects.filter(id__in=ids).delete()
...
def employee_shift_view(request):
    ...
    if apps.is_installed("attendance"):
        GraceTime = get_hr_crux_model_class(app_label="attendance", model="gracetime")
...
def employee_permission_assign(request):
    ...
    installed_apps = [app for app in settings.INSTALLED_APPS if app in hr_crux_apps]
...
def general_settings(request):
    ...
    if apps.is_installed("payroll"):
        PayrollSettings = get_hr_crux_model_class(
            app_label="payroll", model="payrollsettings"
        )
        EncashmentGeneralSettings = get_hr_crux_model_class(
...
@login_required
@permission_required("hr_crux_audit.change_accountblockunblock")
...
@login_required
@permission_required("hr_crux_audit.view_audittag")
...
@login_required
@hx_request_required
@permission_required("hr_crux_audit.add_audittag")
...
@login_required
@hx_request_required
@permission_required("hr_crux_audit.change_audittag")
...
@login_required
def get_hr_crux_installed_apps(request):
