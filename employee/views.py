...
from hr_crux.filters import hr_cruxPaginator
from hr_crux.group_by import group_by_queryset
from hr_crux.hr_crux_settings import hr_crux_DATE_FORMATS
from hr_crux.methods import get_hr_crux_model_class
from hr_crux_audit.models import AccountBlockUnblock, HistoryTrackingFields
from hr_crux_documents.forms import (
    DocumentForm,
    DocumentRejectForm,
    DocumentRequestForm,
    DocumentUpdateForm,
)
from hr_crux_documents.models import Document, DocumentRequest
...
@login_required
@manager_can_enter("hr_crux_documents.view_documentrequests")
def document_request_view(request):
    ...
    documents = filtersubordinates(
        request=request,
        perm="hr_crux_documents.view_documentrequests",
        queryset=documents,
    )
...
@login_required
@hx_request_required
@manager_can_enter("hr_crux_documents.add_documentrequests")
def document_request_create(request):
    ...
    form = choosesubordinates(request, form, "hr_crux_documents.add_documentrequest")
    if request.method == "POST":
        form = DocumentRequestForm(request.POST)
        form = choosesubordinates(
            request, form, "hr_crux_documents.add_documentrequest"
        )
        if form.is_valid():
            form = form.save()
...
@login_required
@hx_request_required
@manager_can_enter("hr_crux_documents.change_documentrequests")
...
@login_required
@hx_request_required
@owner_can_enter("hr_crux_documents.view_document", Employee)
...
@login_required
@hx_request_required
@owner_can_enter("hr_crux_documents.add_document", Employee)
...
def document_delete(request, id):
    ...
    try:
        ...
        if not request.user.has_perm("hr_crux_documents.delete_document"):
...
@login_required
@hx_request_required
@manager_can_enter("hr_crux_documents.add_document")
...
def paginator_qry(qryset, page_number):
    ...
    """
    paginator = hr_cruxPaginator(qryset, get_pagination())
        ...
        for models in related_models:
            ...
            if field_name:
                ...
                if (
                    ...
                    Recruitment = get_hr_crux_model_class(
                    ...
                    OnboardingStage = get_hr_crux_model_class(
                    ...
                    OnboardingTask = get_hr_crux_model_class(
    ...
    for column_value, column_name in selected_columns:
        ...
        for employee in employees:
            ...
            if type(value) == date:
                ...
                for format_name, format_string in hr_crux_DATE_FORMATS.items():
...
def joining_today_count(request):
    ...
    if apps.is_installed("recruitment"):
        Candidate = get_hr_crux_model_class(app_label="recruitment", model="candidate")
...
def bonus_points_tab(request, emp_id):
    ...
    if apps.is_installed("payroll"):
        Reimbursement = get_hr_crux_model_class(
