...
class FilingStatus(hr_cruxModel):
    """
    FilingStatus model
    """
...
class LoanAccount(hr_cruxModel):
    """
...
class Reimbursement(hr_cruxModel):
    """
    Reimbursement Model
    """
            ...
            if self.status == "approved" and self.allowance_id is None:
                if self.type == "reimbursement":
                    ...
                    if bonus_points.points >= self.bonus_to_encash:
                        ...
                        request = getattr(
                            hr_crux_middlewares._thread_locals, "request", None
                        )
                        if request:
                            messages.info(
                                request,
...
class ReimbursementrequestComment(hr_cruxModel):
    """
