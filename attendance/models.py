...
class AttendanceActivity(hr_cruxModel):
    """
...
class BatchAttendance(hr_cruxModel):
    """
...
class AttendanceRequestFile(hr_cruxModel):
    file = models.FileField(upload_to="attendance/request_files")


class AttendanceRequestComment(hr_cruxModel):
    """
...
class AttendanceOverTime(hr_cruxModel):
    """
...
class AttendanceLateComeEarlyOut(hr_cruxModel):
    """
...
class AttendanceValidationCondition(hr_cruxModel):
    """
...
class AttendanceGeneralSetting(hr_cruxModel):
    """
...
if apps.is_installed("leave") and apps.is_installed("payroll"):
    ...
    class PenaltyAccount(hr_cruxModel):
        """
    ...
    def create_initial_stage(sender, instance, created, **kwargs):
        ...
        if created:
            ...
            if penalty_amount:
                Deduction = get_hr_crux_model_class(
...
class WorkRecords(models.Model):
    ...
    def title_message(self):
        ...
        if title_message == "Absent":
            if apps.is_installed("leave"):
                LeaveRequest = get_hr_crux_model_class(
